...
data_for_GBS <- all_data_ex[, sum(ITEMS),
                                   by = c("YEAR", "AGE_BAND", "drug_name", "BNF_CHEMICAL_SUBSTANCE_CODE", "population")]
...
data_for_GBS2 <- data_for_GBS[, sum(V1), by = c("YEAR", "target_drug", "age_binary")]
all_value <- data_for_GBS[, sum(V1), by = c("YEAR", "target_drug")]

data_for_GBS2[all_value, on = c("YEAR", "target_drug" ), prescriptions := i.V1]
data_for_GBS2[age_binary== "<1", prescriptions := V1]
data_for_GBS2[age_binary == "Other", age_binary := "All"]

colnames(data_for_GBS2) <- c("Year", "target_drug", "age_binary","V1", "value")
...
data_for_GBS2[,"V1" := NULL]
data_for_GBS2[, type := "Prescriptions"]
GBS_resistant_m[, type := "Proportion"]

combined_GBS <- rbind(data_for_GBS2, GBS_resistant_m)
